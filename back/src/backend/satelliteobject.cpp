...
SatelliteObject::SatelliteObject(Observer InputObserver)
{
    for (int i = 0; i < 64; i++)
        _max_elevation[i] = -1;
    this->tle = new Tle();
...
void SatelliteObject::Refresh()
{
    if (this->cal)
    {
        ...
        {
            this->callback(SGS_CONMOD_DBG, "%s\t%s\t", this->_name, e.what());
        }
        this->cal = true;
    }
...
void SatelliteObject::GeneratePath()
{
    ...
    for (int i = 0; i < 1023; i++)
    {
        ...
        if (cal > 90.0f)
        {
            ...
            {
                this->callback(SGS_CONMOD_LOG, "Azimuth Crossing Path(Counterclockwise)");
                this->CrossPath = 1;
            }
            ...
            {
                this->callback(SGS_CONMOD_LOG, "Azimuth Crossing Path(Clockwise)");
                this->CrossPath = -1;
            ...
            {
                this->callback(SGS_CONMOD_LOG, "This Path Requires manual control.");
                this->CrossPath = -2;
            }
            break;
        }
