...
void SGS_TLMWindow::Render_Beacon(float fontscale)
{
    ...
    if (ImGui::BeginTabItem("Beacon"))
    {
        ...
        if (ImGui::BeginTable("##BeaconTables", 2,
                                  ...
                                  ImGuiTableFlags_Reorderable))
        {
            ...
            StateWindowColumnManager("Length");
            ImGui::Text("%" PRIu16, csp_ntoh16(beacon->CCLength));
            StateWindowColumnManager("Time Code");
            ImGui::Text("%u %u %u %u %u %u",
                        beacon->CCTime_code[0], beacon->CCTime_code[1], beacon->CCTime_code[2],
                        beacon->CCTime_code[3], beacon->CCTime_code[4], beacon->CCTime_code[5]);

            // ── SRL ────────────────────────────────────────────────────────────────────
            StateWindowColumnManager("SRL Command Counter");
            ImGui::Text("%" PRIu8, beacon->SRL_CommandCounter);
            StateWindowColumnManager("SRL Command Error Counter");
            ImGui::Text("%" PRIu8, beacon->SRL_CommandErrorCounter);
            StateWindowColumnManager("SRL I2C0 Status / TxCount");
            ImGui::Text("%" PRIu8 " / %" PRIu16, beacon->SRL_I2C0_Status, beacon->SRL_I2C0_TxCount);
            StateWindowColumnManager("SRL I2C1 Status / TxCount");
            ImGui::Text("%" PRIu8 " / %" PRIu16, beacon->SRL_I2C1_Status, beacon->SRL_I2C1_TxCount);
            StateWindowColumnManager("SRL I2C2 Status / TxCount");
            ImGui::Text("%" PRIu8 " / %" PRIu16, beacon->SRL_I2C2_Status, beacon->SRL_I2C2_TxCount);
            StateWindowColumnManager("SRL UART Status / TxCount");
            ImGui::Text("%" PRIu8 " / %" PRIu16, beacon->SRL_UART_Status, beacon->SRL_UART_TxCount);
            StateWindowColumnManager("SRL RS422 Status / TxCount");
            ImGui::Text("%" PRIu8 " / %" PRIu16, beacon->SRL_RS422_Status, beacon->SRL_RS422_TxCount);

            // ── RPT ────────────────────────────────────────────────────────────────────
            StateWindowColumnManager("RPT Report Queue / Critical Queue");
            ImGui::Text("%" PRIu8 " / %" PRIu8, beacon->RPT_ReportQueueCount, beacon->RPT_CriticalQueueCount);
            StateWindowColumnManager("RPT Boot Count");
            ImGui::Text("%" PRIu16, beacon->RPT_BootCount);
            StateWindowColumnManager("RPT Spacecraft Time (s)");
            ImGui::Text("%" PRIu32, beacon->RPT_SpacecraftTime_s);
            StateWindowColumnManager("RPT Spacecraft Time (subsec)");
            ImGui::Text("%" PRIu32, beacon->RPT_SpacecraftTime_subs);
            StateWindowColumnManager("RPT Sequence");
            ImGui::Text("%" PRIu32, beacon->RPT_Sequence);
            StateWindowColumnManager("RPT Reset Cause");
            ImGui::Text("%" PRIu8, beacon->RPT_ResetCause);
            StateWindowColumnManager("RPT Current Mode / Previous");
            ImGui::Text("%" PRIu16 " / %" PRIu16, beacon->RPT_CurrentMode, beacon->RPT_PreviousMode);

            // ── STRX ───────────────────────────────────────────────────────────────────
            StateWindowColumnManager("STRX Status");
            ImGui::Text("%" PRIu8, beacon->STRX_status);
            StateWindowColumnManager("STRX Last RSSI (dBm)");
            ImGui::Text("%" PRId16, beacon->STRX_last_rssi);
            StateWindowColumnManager("STRX BootCount / BootCause");
            ImGui::Text("%" PRIu16 " / %" PRIu32, beacon->STRX_bootcount, beacon->STRX_bootcause);
            StateWindowColumnManager("STRX RX Mode");
            ImGui::Text("%" PRIu8, beacon->STRX_rx_mode);
            StateWindowColumnManager("STRX GND WDT Cnt / Left");
            ImGui::Text("%" PRIu16 " / %" PRIu32, beacon->STRX_gnd_wdt_cnt, beacon->STRX_gnd_wdt_left);

            // ── UANT / UTX ─────────────────────────────────────────────────────────────
            StateWindowColumnManager("UANT Deployment Status");
            ImGui::Text("%" PRIu16, beacon->UANT_ReportDeploymentStatus);

            StateWindowColumnManager("UTX Active Conf");
            ImGui::Text("%" PRIu8, beacon->UTX_active_conf);
            StateWindowColumnManager("UTX Boot Count / Cause");
            ImGui::Text("%" PRIu16 " / %" PRIu32, beacon->UTX_boot_count, beacon->UTX_boot_cause);

            // ── EPS ────────────────────────────────────────────────────────────────────
            StateWindowColumnManager("EPS Vbatt (mV)");
            ImGui::Text("%" PRIu16, beacon->EPS_vbatt);
            StateWindowColumnManager("EPS Output[8]");
            ImGui::Text("%u %u %u %u %u %u %u %u",
                        beacon->EPS_output[0], beacon->EPS_output[1], beacon->EPS_output[2], beacon->EPS_output[3],
                        beacon->EPS_output[4], beacon->EPS_output[5], beacon->EPS_output[6], beacon->EPS_output[7]);
            StateWindowColumnManager("EPS CurOut[6] (mA)");
            ImGui::Text("%" PRIu16 " %" PRIu16 " %" PRIu16 " %" PRIu16 " %" PRIu16 " %" PRIu16,
                        beacon->EPS_curout[0], beacon->EPS_curout[1], beacon->EPS_curout[2],
                        beacon->EPS_curout[3], beacon->EPS_curout[4], beacon->EPS_curout[5]);
            StateWindowColumnManager("EPS CurIn[3] (mA)");
            ImGui::Text("%" PRIu16 " %" PRIu16 " %" PRIu16,
                        beacon->EPS_curin[0], beacon->EPS_curin[1], beacon->EPS_curin[2]);
            StateWindowColumnManager("EPS CurSys (mA)");
            ImGui::Text("%" PRIu16, beacon->EPS_cursys);
            StateWindowColumnManager("EPS Boot Counter");
            ImGui::Text("%" PRIu16, beacon->EPS_counter_boot);
            StateWindowColumnManager("EPS GND WDT Time Left");
            ImGui::Text("%" PRIu32, beacon->EPS_wdt_gnd_time_left);
            StateWindowColumnManager("EPS BootCause / BattMode");
            ImGui::Text("%" PRIu8 " / %" PRIu8, beacon->EPS_bootcause, beacon->EPS_battmode);

            // ── Solar ──────────────────────────────────────────────────────────────────
            StateWindowColumnManager("Solar Deploy Status");
            ImGui::Text("%" PRIu8, beacon->Solar_DeployStatus);

            // ── ADCS ───────────────────────────────────────────────────────────────────
            StateWindowColumnManager("ADCS PowerState / ControlMode");
            ImGui::Text("%" PRIu8 " / %" PRIu8, beacon->ADCS_PowerState, beacon->ADCS_ControlMode);
            StateWindowColumnManager("ADCS GyroCalRate X Y Z (rad/s)");
            ImGui::Text("%.6f %.6f %.6f",
                        beacon->ADCS_GyroCalRateX, beacon->ADCS_GyroCalRateY, beacon->ADCS_GyroCalRateZ);

            // ── PAYLOAD ────────────────────────────────────────────────────────────────
            StateWindowColumnManager("PAY CAM Mode / Mem / Image#");
            ImGui::Text("%" PRIu8 " / %" PRIu8 " / %" PRIu8,
                        beacon->PAY_CAM_Mode, beacon->PAY_CAM_MemoryStatus, beacon->PAY_CAM_ImageNumber);
            StateWindowColumnManager("PAY Deorbit ADC");
            ImGui::Text("%" PRIu16, beacon->PAY_DEORBIT_ADCvalue);
