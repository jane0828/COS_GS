...
static void * SGS_IPServerTask(void * param)
{
    ...
    if(ip->clnt_sock == -1)
    	ip->callback(SGS_CONMOD_ERR, "ServerTask SocketAccept Failure.");
...
int SGS_IP::InitializeServer()
{
    ...
    {
        this->callback(SGS_CONMOD_ERR, "Socket Creation Error\n");
        return -1;
    }
    ...
    {
        this->callback(SGS_CONMOD_ERR, "Bind Failed.\n");
        return -3;
    }
    
    if(listen(this->descripter, 3) < 0)
    {
        this->callback(SGS_CONMOD_ERR, "Listen Error.\n");
        return -4;
    }
...
int SGS_IP::InitializeClient()
{
    ...
    if(this->clnt_sock == -1)
        this->callback(SGS_CONMOD_ERR, "ClientTask SocketAccept Failure.");
