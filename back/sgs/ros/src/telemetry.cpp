...
int TelemetryHandler::dispatch_payload(const uint8_t* payload, size_t len) {
    ...
    if (n >= sizeof(Beacon) + 4) { // Beacon(143) + inner_crc(4)
        uint16_t msgid_beacon = ntohs(*(const uint16_t*)(p)); // Beacon의 첫 2바이트는 msgid
        if (msgid_beacon == BEACON_CC_ID) {
            std::fprintf(stderr, "[tm] beacon path (no secondary header)\n");
            // p: Beacon 시작, 길이는 정확히 sizeof(Beacon)
            handle_beacon_direct(p, sizeof(Beacon));

            // 소비: Beacon(143) + inner_crc(4)
            size_t consumed = sizeof(Beacon) + 4;
            if (n < consumed) {
                std::fprintf(stderr, "[tm.bec] BREAK: n(%zu) < consumed(%zu)\n", n, consumed);
                return 0;
            }
            p += consumed;
            n -= consumed;
            ...
            // 남은 바이트에 다른 프레임이 연달아 붙었을 수 있으니, 기존 루프로 이어가도 되고
            // 여기서 끝내도 됨. 우리는 이어가자.
        }
    }
    ...
    while (n >= kSecHdr) {
        ...
        if (ccid == IMAGE_CC_ID) {
            ...
            bool is_last_flag = (ccseq & 0x8000) != 0;   // 상위비트로 LAST 표시되는 경우 (예: 0x8075)
            // bool is_short_tail = (img_len < kImgPayload); // 128B보다 짧으면 마지막 꼬리로 판단

            // if (is_last_flag || is_short_tail) {
            if (is_last_flag && !saved_once) {
                g_bitstream.save_final();  // 지금까지 누적된 모든 바이트를 1개 파일로 저장
                g_bitstream.clear();       // 다음 이미지 준비
                saved_once = true;
            }
        ...
        else if (ccid == REPORT1_CC_ID || ccid == REPORT2_CC_ID) {
            std::fprintf(stderr, "[tm.rep] app_n=%zu need=%zu\n", app_n, sizeof(Report));

            // Case A: (희귀) 2차 헤더가 실제로 있는 스트림
            if (app_n >= sizeof(Report)) {
                // 기존 로직 유지
                handle_report_direct(app, sizeof(Report));
                size_t consumed = kSecHdr + sizeof(Report) + 4; // tail 4B 가정
                if (n < consumed) { std::fprintf(stderr, "[tm.rep] BREAK A: n(%zu) < consumed(%zu)\n", n, consumed); break; }
                p += consumed; n -= consumed;
                std::fprintf(stderr, "[tm.rep] consumed(A)=%zu, remain=%zu\n", consumed, n);
                continue;
            }

            // Case B: (일반) 2차 헤더가 없는 스트림
            {
                // after_CSP 전체 길이
                size_t after_csp = n;           // p는 CSP 뒤 첫 바이트
                const size_t need_min = sizeof(Report) + 4; // Report 152 + tail 4
                if (after_csp < need_min) {
                    std::fprintf(stderr, "[tm.rep] too short (no-sec-hdr): after_csp=%zu need>=%zu\n",
                                after_csp, need_min);
                    return -22;
                }

                // pad = after_CSP - Report(152) - tail(4)
                size_t pad = after_csp - sizeof(Report) - 4;
                std::fprintf(stderr, "[tm.rep] no-sec-hdr: pad=%zu (after_csp=%zu)\n", pad, after_csp);

                // Report는 p에서 바로 152바이트
                handle_report_direct(p, sizeof(Report));

                size_t consumed = sizeof(Report) + pad + 4; // report + padding + tail
                if (n < consumed) { std::fprintf(stderr, "[tm.rep] BREAK B: n(%zu) < consumed(%zu)\n", n, consumed); break; }
                p += consumed; n -= consumed;
                std::fprintf(stderr, "[tm.rep] consumed(B)=%zu, remain=%zu\n", consumed, n);
                continue;
            }
        }
        else {
...
int TelemetryHandler::SaveRaw(const uint8_t* buf, size_t len) {
    ...
    std::snprintf(path, sizeof(path),
                  "./data/raw/Telemetry--%04d-%02d-%02d-%02d-%02d-%02d.bin",
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        std::fprintf(fp, "%hhx", buf[i]);   // changed to parse image
    }
    std::fprintf(fp, "\n");
    std::fclose(fp);
