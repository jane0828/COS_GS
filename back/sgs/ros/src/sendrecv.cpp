...
static std::string save_raw_txt_always(const uint8_t* buf, size_t len) {
    ...
    char path[256];
    std::snprintf(path, sizeof(path),
                //   "./data/raw/raw--%04d-%02d-%02d-%02d-%02d-%02d.txt",      // original
                  "./data/raw/raw--%04d-%02d-%02d-%02d-%02d-%02d.bin",
                  lt->tm_year + 1900, lt->tm_mon + 1, lt->tm_mday,
                  lt->tm_hour, lt->tm_min, lt->tm_sec);
    // FILE* fp = std::fopen(path, "w"); // original
    FILE* fp = std::fopen(path, "wb");
    if (!fp) { perror(path); return {}; }
    
    
    
    size_t written = std::fwrite(buf, 1, len, fp);
    if (written != len) {
        perror("fwrite");
        std::fclose(fp);
        return {};
    ...
    out_path = path;
    std::printf("[recv][raw] saved binary %zu bytes to %s\n", len, path);
    return out_path;
...
void SendRecv::set_target(const std::string& ip, int port) {
    ip_ = ip;
    port_ = port;
}
...
int SendRecv::start_recv_tcp(std::atomic<bool>& run) {
    ...
    while (run.load(std::memory_order_relaxed)) {
        ...
        raddr.sin_port        = htons((uint16_t)port_);
        raddr.sin_addr.s_addr = inet_addr(ip_.c_str());
