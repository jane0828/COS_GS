...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative flex h-screen bg-gray-900">
      {/* 🛰️ 사이드바 */}
      <div className={`absolute left-0 top-0 h-full z-50 shadow-lg transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-16'}`}>
        <SideNav />
      </div>

      {/* 🌌 콘텐츠 영역 */}
      <div className={`flex-grow min-h-screen transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-16'} ${getBackgournd_GS()}`}>
        {children}
      </div>
    </div>
  );
